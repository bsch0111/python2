...
#%%
# 첫행 삭제
data=pd.read_csv("C:\\inputcsv\\result_csv\\PAINTING_BABY_result.csv")
#%%
...
for point1 in data_sorted_keys.index:
    for point2 in data_sorted_keys.index:
        p_sum = 0
        for element in data_sorted_keys[point2] :
                p_sum = p_sum + data_sorted_keys[point1].count(element)
        data_count[point1][point2] = p_sum

# %%
import datetime
import os
basename3 = os.getcwd()+'\\XRF\\XRF_no_ordering_matching_'+datetime.datetime.now().strftime("%y%m%d_%H%M%S")
